...
def parseBlif(rfilename, wfilename):
    ...
    for gate in gates:
        ...
        if gate_type in ["t", "ccx"]:
            for (position, ith_qubit) in enumerate(qubits):
                # fanin
                if gate_type == "t":
                    clifford_observables[qubit_mapping[ith_qubit]] = 'output of cut %d %s' % (n_cut, gate_type)
                elif gate_type == "ccx":
                    clifford_observables[qubit_mapping[ith_qubit]] = 'output of cut %d %s' % (n_cut, gate_type)
    ...
    return (n_t + n_ccx * 3, n_qubits, n_qubits + n_ancilla, t_gates, ccx_gates, clifford_observables, tableau)
