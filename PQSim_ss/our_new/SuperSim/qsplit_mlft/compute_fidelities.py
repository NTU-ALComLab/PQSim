...
from circuit_translator_2 import get_fragments_from_file
...
def get_profile_from_prebuilt(
    fragments: Dict[str, cm.Fragment],
    repetitions: int,
) -> Dict[Tuple[int, ...], float]:
    """
    Compute the circuit profile from prebuilt fragments using.
    1. the maximum likelyhood fragment tomography method in arXiv:2005.12702.
    """
    # get all qubits in all fragments that are not quamtum_inputs
    qubit_order = []
    for fragment in fragments.values():
        qubit_order += [qubit for qubit in fragment.circuit.all_qubits() if qubit not in fragment.quantum_inputs]
    qubit_order = sorted(qubit_order)
    # print('qubit_order')
    # print(qubit_order)
    # print('end qubit_order')
...
if __name__ == "__main__":
    ...
    num_clusters = 4
    repetitions = 10**4
    fragments = get_fragments_from_file("test_out.qasm", "test_out_tp.qasm")
    start = time.time()
    profile = get_profile_from_prebuilt(fragments, repetitions)
    end = time.time()
    print("Time taken: ", end-start)
    # turn the keys of the profile into strings
