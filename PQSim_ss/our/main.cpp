...
void query_mode(string filename, int n_origin, int n_after, int n_cuts, string query) {
    string ccx_filename = "Toffoli.csv";
    Observable obs(filename, ccx_filename, n_after, n_cuts, n_origin);
...
void sample_mode(string filename, int n_origin, int n_after, int n_cuts, int total_shot) {
...
int main(int argc, char* argv[]){
    if (argc != 7) {
        std::cerr << "Usage: " << argv[0] << " <MODE> <PAULI OR SHOTS> <N_ORIGIN> <N_AFTER> <N_CUTS> <OBSERVABLE_MAP_FILE>" << std::endl;
        return 1;
    }
    ...
    */
    ...
    if(mode == "query") {
        query_mode(observable_file, n_origin, n_after, n_cuts, pauli_or_shots);
    } else if(mode == "sample") {
        sample_mode(observable_file, n_origin, n_after, n_cuts, std::stoi(pauli_or_shots));
    } else {
